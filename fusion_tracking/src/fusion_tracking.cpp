...
void track_cb (const sensor_msgs::PointCloud2ConstPtr& input)
{
    ...
    if (model_ready == true) {
        ...
        // Publishes XYZ Coordinates of the Person
        std::cout << movement_message << std::endl;

        // Visualizes the Particle Tracking
        ...
        // Publishes the Cloud for Visualization
...
void model_cb (const sensor_msgs::PointCloud2ConstPtr& input)
{
    ...
    // Gets Model for Particle Filter
    pcl_conversions::toPCL(*input, input_cloud);
	pcl::fromPCLPointCloud2(input_cloud, *cloud);
    std::vector<int> indices;
    pcl::removeNaNFromPointCloud(*cloud,*cloud, indices);
...
int main (int argc, char** argv)
{
    ...
    // Sets Up Coherence Parameters for Tracking
	...
	distance_coherence->setWeight(8);
	...
	coherence->setMaximumDistance (0.07); 

	tracker_->setCloudCoherence (coherence);
  
    // Initialize ROS
    ros::init (argc, argv, "fusion_pf");
    ros::NodeHandle nh;

    // Create a ROS subscriber for the input point cloud
    ros::Subscriber sub = nh.subscribe ("/icp_output", 1, track_cb);
    ros::Subscriber sub_model = nh.subscribe ("/fusion_correct_person_output", 1, model_cb);
