...
void track_cb (const sensor_msgs::PointCloud2ConstPtr& input)
{
    ...
    if (model_ready == true) {
        ...
        // publishes xyz of the person
        std::cout << movement_message << std::endl;
        
        // visualization of the clouds
        ...
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr input_conv (new pcl::PointCloud<pcl::PointXYZRGB> ());
...
int main (int argc, char** argv)
{
    ...
    // Initialize ROS
    ros::init (argc, argv, "lidar_particle_filter");
    ros::NodeHandle nh;

    // Create a ROS subscriber for the input point cloud
    ros::Subscriber sub = nh.subscribe ("/solo_voxel_lidar", 1, track_cb);
    ros::Subscriber sub_model = nh.subscribe ("/lidar_person_output", 1, model_cb);

    // Create a ROS publisher for the output point cloud
    pub = nh.advertise<geometry_msgs::Vector3> ("/movement", 1);
    pub2 = nh.advertise<sensor_msgs::PointCloud2> ("lidar_particle_output", 1);
