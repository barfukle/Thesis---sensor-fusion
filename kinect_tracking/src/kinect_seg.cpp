...
void segment(const sensor_msgs::PointCloud2ConstPtr& input)
{
    ...
    passthru.setFilterLimits(0.5, 2); //0.75,2
...
main (int argc, char** argv)
{
  ...
  ros::Subscriber sub = nh.subscribe ("/kinect_voxel_output", 1, segment);

  // Create a ROS publisher for the output point cloud
  pub = nh.advertise<sensor_msgs::PointCloud2> ("kinect_plane_output", 1);
  pub2 = nh.advertise<sensor_msgs::PointCloud2> ("kinect_seg_output", 1);
  test1 = nh.advertise<sensor_msgs::PointCloud2> ("kinect_test_output", 1);
