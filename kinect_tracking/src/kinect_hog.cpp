...
void GroundFinder (const sensor_msgs::PointCloud2ConstPtr& input)
{
    ...
    */
    ...
    /*
    PointCloudT::Ptr test_2 (new PointCloudT);
    pcl::ExtractIndices<pcl::PointXYZRGBA> extract2;
    extract2.setInputCloud(input_cloud);
    extract2.setIndices(inliers);
    extract2.setNegative (true);
    extract2.filter(*test_2);
    
    
    sensor_msgs::PointCloud2::Ptr output (new sensor_msgs::PointCloud2);
    pcl::toROSMsg(*test_2, *output);
    output->header.frame_id = input->header.frame_id;
    test1.publish(output);
    */
    // Person Detection  
    ...
    for(std::vector<pcl::people::PersonCluster<pcl::PointXYZRGBA> >::iterator it = clusters.begin(); it != clusters.end(); ++it)
    {
        if(it->getPersonConfidence() > -20)             
        {
            ...
            *visual_cloud = *visual_cloud + *subset_cloud;
            ...
            sensor_msgs::PointCloud2::Ptr output2 (new sensor_msgs::PointCloud2);
            pcl::toROSMsg(*visual_cloud, *output2);
            output2->header.frame_id = input->header.frame_id;
            test2.publish(output2);
...
int main (int argc, char** argv)
{
    ...
    // Initialize ROS
    ros::init (argc, argv, "kinect_hog");
    ros::NodeHandle nh;

    // Create a ROS subscriber for the input point cloud
    ros::Subscriber sub = nh.subscribe ("/kinect_voxel_output", 1, GroundFinder);

    // Create a ROS publisher for the output point cloud
    pub = nh.advertise<sensor_msgs::PointCloud2> ("/kinect_hog_output", 1);
